...
| **Session Key - (Used as a Mock)** | Random 256-bit key generated by the platform | Encrypts all sensor data (AES-GCM or AES-CBC+HMAC). Rotated periodically. |
...
| `AES-CBC` | Split: first 16 bytes → `session_key` (encryption), last 16 bytes → `mac_key` (HMAC-SHA256) |
