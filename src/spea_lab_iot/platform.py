...
After enrollment, performs authenticated DH key agreement (R4) to derive session keys.
"""
...
def run_platform(log_enabled: bool = False) -> None:
    ...
    # session_keys: device_id -> 32-byte session key (populated after DH handshake)
    # auth_keys   : device_id -> 32-byte auth key    (populated after DH handshake)
    # R5 will read from both dicts to encrypt/decrypt data
    session_keys: dict[str, bytes] = {}
    auth_keys: dict[str, bytes] = {}

    # R4: DH handler — shared allowed_devices so it always sees latest pins
    ...
    def on_data_message(
        ...
        # R4: warn if DH handshake not yet completed
        if device_id not in session_keys:
            _log(log_mode[0], f"Warning: no session key yet for device_id={device_id!r}")

        # TODO (R5): decrypt payload using session_keys[device_id] before relaying
        ...
        while True:
            ...
            if choice == "1":
                ...
                if did == ALLOWED_DEVICES_KEY_DEFAULT:
                    ...
                    session_keys.pop(did, None)  # R4: clear session key on removal
                    auth_keys.pop(did, None)      # R4: clear auth key on removal
                    print(
                ...
                # R4: show which devices have an active session key
                if session_keys:
